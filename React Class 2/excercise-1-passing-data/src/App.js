...
class App extends Component {
  showPeopleInterestsBooks(data) {
    const name = peeps[data.peepsID].name;
    const book = books[data.bookLikedID].name;
    return {
      txt: `${name}'s likes a ${book} book`,
      key: data.id
    };
  }
  render() {
    return (
      <div>
        ...
        <List
          data={chart.map(this.showPeopleInterestsBooks)}
        />
      </div>
    );
  }
